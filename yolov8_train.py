...
class YOLOv8TrafficSignTrainer:
    ...
    def train_model(self, yaml_config, epochs=50, imgsz=640, batch=32, device='auto'):
        """
        YOLOv8 모델 학습
...
def main():
    """메인 실행 함수"""
    # 트레이너 초기화 (nano 모델 사용으로 Raspberry Pi 최적화)
    # yolo_data 디렉토리 경로 수정
    trainer = YOLOv8TrafficSignTrainer(data_dir='/Users/moss/PycharmProjects/PythonProject/training/yolo_data', model_size='s')

    # 데이터셋 준비
    # yolo_data 폴더에 이미 준비된 파일들을 사용
    yaml_config = '/Users/moss/PycharmProjects/PythonProject/training/yolo_data/dataset.yaml'

    # 모델 학습
    # device를 'auto'에서 'mps'로 변경하여 M4 Pro의 GPU를 사용하도록 함
    results = trainer.train_model(
        yaml_config=yaml_config,
        epochs=10,
        imgsz=320,  # Raspberry Pi에 최적화된 크기
        batch=32,  # 메모리에 따라 조정
        device='mps'  # Mac용 GPU(Metal) 사용으로 변경
    )

    # 최적 모델로 검증
    # best.pt 모델 파일 경로 수정
    best_model_path = '/Users/moss/PycharmProjects/PythonProject/training/traffic_sign_detection/yolov8_training/weights/best.pt'
